...
scalar(type = "int8", quantile = 0.8, TRUE) |>
...
quantization <- function(binary = NULL, product = NULL, scalar = NULL) {
  ...
  structure(list(quantization_config = params), class = c("config", "quantization"))
}

quantization(binary = binary(TRUE), product = product("x4", FALSE))
